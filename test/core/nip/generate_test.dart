  ...
  group('generate', () {
    test('generate produces standard NIP for child at age 0', () {
      ...
      for (final due in dues) {
        ...
        expect(today.add(getDoseAge(due.vaccineCode, due.doseNumber)!.duration) == due.dueDate, true);
      }
    });
    ...
    test('generate does not produce dues for existing records', () {
      ...
      for (final record in records) {
        expect(dues.any((GeneratedDue due) => due.vaccineCode == record.vaccineCode && due.doseNumber == record.doseNumber), false);
      }
    });
    ...
    test('generate calculates the catch-up schedule for overdue vaccinations', () {
      ...
      expect(bopvDues.length, 2, reason: 'Expected 2 BOPV dues to be created, instead found ${bopvDues.length}');
      expect(bopvDues.any((due) => due.dueDate == dob.add(getDoseAge('BOPV', 1)!.duration)), false);
      expect(bopvDues.any((due) => due.dueDate == today.add(const Duration(days: 1))), true);
      expect(bopvDues.any((due) => due.dueDate == today.add(DayDuration(months: 1).duration + const Duration(days: 1))), true);
    });
    ...
    test('generate does not use catch-up schedule for ongoing vaccinations', () {
      ...
      expect(bopvDues.any((due) => due.dueDate == dob.add(getDoseAge('BOPV', 2)!.duration)), true);
      expect(bopvDues.any((due) => due.dueDate == dob.add(getDoseAge('BOPV', 3)!.duration)), true);
    });
  });
